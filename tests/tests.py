...
class BagViewTests(TestCase):
    ...
    def test_add_to_bag(self):
        response = self.client.post(reverse('add_to_bag'), data=json.dumps({
    ...
    def test_remove_from_bag(self):
        self.client.session['bag'] = [{'session': {'id': self.session.id, 'title': self.session.title, 'price': self.session.price}}]
        self.client.session.save()

        response = self.client.post(reverse('remove_from_bag'), data=json.dumps({'session_id': self.session.id}), content_type='application/json')
